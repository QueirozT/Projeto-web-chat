...
from display import *
# Criando o web-scraper para obter a url do servidor WebSocket de teste
from web_scraper_key import gerar_link
# Criando uma encriptação para enviar mensagens
import cryptocode
        ...
        async for ws in websockets.connect(self.url):
            while True:
                try:
                    ...
                    await ws.send(cryptocode.encrypt(f'PING', CODE))
                    await ws.recv()
                    ...
                    while tempo + 30 > round(time.time()):
                        if self.msg:
                            await ws.send(cryptocode.encrypt(f'ENCRIPTED_MESSAGE:{self.msg}', CODE))
                            await ws.recv()
                            self.msg = ''
                            await asyncio.sleep(0.2)
    ...
    async def consume(self, url: str) -> None:
        async for ws in websockets.connect(self.url):
            try:
                async for msg in ws:
                    try:
                        msg = cryptocode.decrypt(msg, CODE)
                    except:
                        pass
                    else:
                        try:
                            if "ENCRIPTED_MESSAGE:" in msg:
                                msg = msg.replace("ENCRIPTED_MESSAGE:", "")
                                if msg != 'PING':
